...
const TransactionItem: FC<Props> = ({ amount, date, type }) => {
  ...
  return (
    <Card className={cn("p-1")}>
      <CardHeader className='flex justify-between items-center p-1.5 rounded-md'>
        <div className={cn(`border rounded-full p-2 ${outcomeTransaction ? "bg-[#d8eee1]" : "bg-[#f5b0b0]"}`)}>
          {outcomeTransaction ? (
        ...
        <div className='flex flex-col gap-1 text-sm'>
          <Label className='amount'>{formatPrice(amount, { hasCurrencySymbol: true })}</Label>
